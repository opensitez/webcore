...
pub(crate) fn parse_animation_element(node: &SvgNode) -> Option<SvgAnimationElement> {
    ...
    Some(SvgAnimationElement {
        ...
        calc_mode: parse_calc_mode(node.attr_ascii_case_insensitive("calcMode")),
...
fn parse_time_value(raw: &str) -> Option<SvgAnimationTime> {
    ...
    if value.ends_with("ms") {
        let n = value[..value.len() - 2].trim().parse::<f32>().ok()?;
        return Some(SvgAnimationTime::Seconds(n / 1000.0));
    }
    if value.ends_with('s') {
        let n = value[..value.len() - 1].trim().parse::<f32>().ok()?;
        return Some(SvgAnimationTime::Seconds(n));
    }
    if value.chars().any(|ch| ch.is_ascii_alphabetic()) {
        return Some(SvgAnimationTime::Event(value.to_string()));
    }
...
fn collect_animation_event_controls(
    ...
    if let Some(anim) = &node.animation {
        let target_path = anim
            ...
            .map(|(_, path, _)| path.clone())
            .or_else(|| parent_path.clone());
        if target_path.as_deref() == Some(event_target_path) {
            if animation_times_include_event(&anim.begin, event_name) {
                out.push((path.clone(), "begin".to_string(), elapsed_s));
            }
            if animation_times_include_event(&anim.end, event_name) {
                out.push((path.clone(), "end".to_string(), elapsed_s));
            }
        }
    }
...
fn animation_times_include_event(times: &[SvgAnimationTime], event_name: &str) -> bool {
    times.iter().any(|time| match time {
        SvgAnimationTime::Event(value) => value.eq_ignore_ascii_case(event_name),
        _ => false,
    })
}
...
fn sample_animation(
    ...
    if local_s > active_s {
        return (anim.fill_mode == SvgAnimationFillMode::Freeze)
            .then(|| final_animation_value(anim))
            .flatten();
    }

    let progress = iteration_progress(local_s, dur_s, active_s);
    let iteration = if dur_s > 0.0 {
...
fn resolve_time_seconds(
    ...
    match time {
        SvgAnimationTime::Seconds(s) => Some(*s),
        SvgAnimationTime::Indefinite | SvgAnimationTime::Event(_) => None,
...
fn repeat_duration_seconds(anim: &SvgAnimationElement) -> Option<f32> {
    match anim.repeat_dur.as_ref()? {
        ...
        SvgAnimationTime::Event(_) | SvgAnimationTime::Syncbase { .. } => None,
    }
}
...
fn sample_animation_at_progress(
    ...
    match (&anim.values.from, &anim.values.to, &anim.values.by) {
        ...
        (None, Some(to), _) => {
            if matches!(anim.calc_mode, SvgCalcMode::Discrete) || progress >= 1.0 {
                Some(to.clone())
            } else {
                Some(to.clone())
            }
...
fn final_animation_value(anim: &SvgAnimationElement) -> Option<String> {
    anim.values
        .values
        .last()
        .cloned()
        .or_else(|| anim.values.to.clone())
        .or_else(|| anim.values.by.clone())
}
...
fn animation_delta(anim: &SvgAnimationElement) -> Option<String> {
    let (first, last) = if !anim.values.values.is_empty() {
        (anim.values.values.first()?, anim.values.values.last()?)
    } else {
        (anim.values.from.as_ref()?, anim.values.to.as_ref()?)
    };
    subtract_numeric_values(last, first)
}
