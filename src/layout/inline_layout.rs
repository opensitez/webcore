...
pub fn layout_inline_block(
    ...
    if !matches!(node.style.float, Float::None) {
        margin_left = if left_is_auto { 0.0 } else { rbox.margin_left };
        margin_right = if right_is_auto {
            0.0
        } else {
            rbox.margin_right
        };
    } else if is_block_box && !node.style.width.is_auto() && (left_is_auto || right_is_auto) {
    ...
    // Text decoration is not inherited by the cascade, but CSS paints
    // decorations generated by an inline box across its descendant inline
    // content. Carry that painting source separately from normal inheritance.
    let container_decoration_source = if node.style.text_decoration.underline
...
fn collect_items_inner(
    ...
    if node.is_text_node() {
        ...
        if !rendered_text.is_empty() {
            ...
            // Carry inline paint metadata from the nearest decorating/linking
            // ancestor. This is deliberately not cascade inheritance:
            // `text-decoration` stays non-inherited in computed style, but the
            // decorating box paints its line through descendant inline content.
